...
const NavItem = ({ href, icon: Icon, label, subItems }: NavItemProps) => {
  ...
  // State for expanding the dropdown
  // Auto-expand if a child is active
  const [isOpen, setIsOpen] = useState(isChildActive);

  // If it has subItems, the parent itself might not have a direct href, or we treat it as a toggler
  const isActive = href
    ? (href === "/dashboard" ? pathname === href : pathname.startsWith(href))
    : isChildActive;

  // Handle click for subitem parents
  ...
  return (
    <div className="mb-1">
      ...
      >
        ...
        {/* Active indicator dot for items without children active */}
        {!subItems && isActive && (
          ...
          <div className="min-h-0 border-l border-slate-700/50 ml-4 space-y-1">
            {subItems.map((child) => {
              ...
              return (
                ...
                >
                  {/* Line indicator for active sub-item */}
                  {isSubActive && (
    ...
    <aside className="fixed left-0 top-0 h-screen w-64 bg-[#0B1120] border-r border-slate-800/50 flex flex-col z-50">
      ...
      <nav className="flex-1 px-3 space-y-2 py-6 overflow-y-auto custom-scrollbar">
        ...
        <NavItem
          ...
          label="Universities"
          subItems={[
            { href: "/universities", label: "List Universities", icon: List },
            { href: "/universities/create", label: "Create University", icon: PlusCircle },
          ]}
        />
        ...
        <NavItem
          ...
          label="Scholarships"
          subItems={[
            { href: "/scholarships", label: "List Scholarships", icon: List },
            { href: "/scholarships/create", label: "Create Scholarship", icon: PlusCircle },
          ]}
        />
        ...
        <NavItem
          ...
          label="Programs"
          subItems={[
            { href: "/programs", label: "List Programs", icon: List },
            { href: "/programs/create", label: "Create Program", icon: PlusCircle },
          ]}
        />

        <NavItem href="/faculties" icon={Award} label="Faculties" />
